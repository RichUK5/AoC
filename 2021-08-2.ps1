...
class Display {
    ...
    # 2 = 5 segments and no bottom right
    # 3 = 5 segments and both top right and bottom right
    # 5 = 5 segments and no top right

    # 0 = 6 segments and both top right and bottom left or no middle
    # 6 = 6 segments and no top right
    # 9 = 6 segments and no bottom left

    # 8 = 7 segments

    # top left = 5 segments group with 1, plus one char from 2 segment group
    # bottom 


    Display([string[]]$String) {
        
        $this.Patterns = ($String -split " \| ")[0] -split " "
        $this.Output = ($String -split " \| ")[1] -split " "
        
        $this.2 = $this.Patterns | Where-Object { $_.Length -eq 2}
        $this.3 = $this.Patterns | Where-Object { $_.Length -eq 3}
        $this.4 = $this.Patterns | Where-Object { $_.Length -eq 4}
        $this.5 = $this.Patterns | Where-Object { $_.Length -eq 5}
        $this.6 = $this.Patterns | Where-Object { $_.Length -eq 6}
        $this.7 = $this.Patterns | Where-Object { $_.Length -eq 7}
        
        # Get the strings of 6 chars, group them, only keep the chars that exist in 2/3 strings
        $6GroupChars = (($this.6 -split "" |
            Group-Object |
            Where-Object { $_.Count -eq 2 }).Group |
            Select-Object -Unique) -join ""
        
        # Compare with digit 1, matching position is top right
        $null = $6GroupChars  -match "[$($this.2)]"
        $this.TopRight = $Matches[0]
        
        # Compare with digit 4, not matching position is bottom left
        $this.BottomLeft = $6GroupChars -replace "[$($this.4)]"
        
        # Compare digit 1 with top right, not matching position is bottom right
        $this.BottomRight = $this.2 -replace $this.TopRight,""

        # Work out each digit
        $this.OutputTranslated = [int[]]::new($this.Output.Length)
        for ($i = 0; $i -lt $this.Output.Count; $i++) {
            switch ($this.Output[$i].Length) {
                2 { $this.OutputTranslated[$i] = 1 }
                3 { $this.OutputTranslated[$i] = 7 }
                4 { $this.OutputTranslated[$i] = 4 }
                5 {
                    if ($this.Output[$i] -notlike "*$($this.BottomRight)*") { $this.OutputTranslated[$i] = 2 }
                    elseif ($this.Output[$i] -notlike "*$($this.TopRight)*") { $this.OutputTranslated[$i] = 5 }
                    else { $this.OutputTranslated[$i] = 3 }
                }
                6 {
                    if ($this.Output[$i] -notlike "*$($this.TopRight)*") { $this.OutputTranslated[$i] = 6 }
                    elseif ($this.Output[$i] -notlike "*$($this.BottomLeft)*") { $this.OutputTranslated[$i] = 9 }
                    else { $this.OutputTranslated[$i] = 0 }
                }
                7 { $this.OutputTranslated[$i] = 8 }
                
            }
        }

        # Join each digit into a single int
        $this.Decoded = $this.OutputTranslated -join ""
        
    }
}

[string[]]$Array = $PuzzleData -split "`r?`n"
[Display[]]$Displays = $Array | ForEach-Object {[Display]::new($_)}

# $Displays | ft "2","3","4","5","6","7"
# $Displays | ft TopRight,Middle,BottomLeft,BottomRight,Bottom

return ($Displays.Decoded | Measure-Object -Sum).Sum
