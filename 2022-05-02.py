...
def MoveCrates2(Stacks, InstructionsString):
    Instructions = InstructionsString.splitlines()
    for Instruction in Instructions:
        QtyString, Src, Dst = Instruction.split(" ")[1::2]
        Qty = int(QtyString)
        ...
        while Qty > 0:
            Qty = Qty - 1
            Crane.append(SrcStack.Crates.pop())
        while len(Crane) > 0:
            DstStack.Crates.append(Crane.pop())
...
def Part2(String):
    StacksString, InstructionsString = SplitBlocks(String)
    Stacks = PlaceCrates(StacksString)
    MoveCrates2(Stacks, InstructionsString)
    return "".join([o.Crates[-1] for o in Stacks])
...
class Test(unittest.TestCase):
    ...
    def test_Part2_1(self):
        self.assertEqual('MCD',Part2(TestData))
